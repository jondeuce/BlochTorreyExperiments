...
% div(D*grad(x)) with backward divergence/forward gradient
MxF = ~xor(m, circshift(m, -1, 1));
MyF = ~xor(m, circshift(m, -1, 2));
MzF = ~xor(m, circshift(m, -1, 3));
MxB = ~xor(m, circshift(m, +1, 1));
MyB = ~xor(m, circshift(m, +1, 2));
MzB = ~xor(m, circshift(m, +1, 3));
                
phiF = MxF .* fwd_av(D,1) .* fwd_diff(x,1) + ...
       MyF .* fwd_av(D,2) .* fwd_diff(x,2) + ...
       MzF .* fwd_av(D,3) .* fwd_diff(x,3);
phiB = MxB .* bwd_av(D,1) .* bwd_diff(x,1) + ...
       MyB .* bwd_av(D,2) .* bwd_diff(x,2) + ...
       MzB .* bwd_av(D,3) .* bwd_diff(x,3);
z = phiF - phiB;
end
