function y = SevenPointSum_Brute(x, kern, gsize3D, iters, mask)
%SEVENPOINTDIFFERENCE_BRUTE SevenPointSum_Brute(x, kern, gsize3D, iters)
% General seven point sum operator applied to the array x, with possible
% masking. The sum is weighted by the kernel, and only included if both
% boolean mask values are the same at each sum point. That is,
%
...
y = sevenpointsum_brute(x, kern, mask);
for ii = 2:iters
    y = sevenpointsum_brute(y, kern, mask);
end

end

function y = sevenpointsum_brute(x, kern, m)
...
y = SevenPointSumMasked(x,kern,gsize,iters,mask);
yb = SevenPointSum_Brute(x,kern,gsize,iters,mask);
b = test_approx_eq(y,yb,'cplx-cplx') && b;

y = SevenPointSumMasked(x,real(kern),gsize,iters,mask);
yb = SevenPointSum_Brute(x,real(kern),gsize,iters,mask);
b = test_approx_eq(y,yb,'cplx-real') && b;

y = SevenPointSumMasked(real(x),kern,gsize,iters,mask);
yb = SevenPointSum_Brute(real(x),kern,gsize,iters,mask);
b = test_approx_eq(y,yb,'real-cplx') && b;

y = SevenPointSumMasked(real(x),real(kern),gsize,iters,mask);
yb = SevenPointSum_Brute(real(x),real(kern),gsize,iters,mask);
