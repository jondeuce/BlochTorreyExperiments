function [ varargout ] = plotDeoxyIntra( R, varargin )
%PLOT
...
for idx = 1:numel(deoxysignalsIntra)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,deoxysignalsIntra{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, deoxysignalsIntra);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,deoxysignalsIntra,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(deoxysignalsIntra(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
deoxysignalsIntra = opts.scalefactor * deoxysignalsIntra;
...
hold on
hx = plot(gca, echotimes, deoxysignalsIntra, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',deoxysignalsIntra.');
            ...
            ymax = max( max(deoxysignalsIntra(:)), max(boldinterps(:)) );
            ymin = min( min(deoxysignalsIntra(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(deoxysignalsIntra,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(deoxysignalsIntra(:)), max(max(boldinterps)) );
            ymin = min( min(deoxysignalsIntra(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, deoxysignalsIntra);
    ymax = max(deoxysignalsIntra(:));
    ymin = min(deoxysignalsIntra(:));
end
ylim([ymin,ymax]);
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(deoxysignalsIntra)
        deoxysignalsIntra{idx} = deoxysignalsIntra{idx}(TEidx, alphaidx);
    end
end
