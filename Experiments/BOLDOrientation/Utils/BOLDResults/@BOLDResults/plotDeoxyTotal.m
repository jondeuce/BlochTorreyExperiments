function [ varargout ] = plotDeoxyTotal( R, varargin )
%PLOT
...
for idx = 1:numel(deoxysignals)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,deoxysignals{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, deoxysignals);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,deoxysignals,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(deoxysignals(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
deoxysignals = opts.scalefactor * deoxysignals;
...
hold on
hx = plot(gca, echotimes, deoxysignals, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',deoxysignals.');
            ...
            ymax = max( max(deoxysignals(:)), max(boldinterps(:)) );
            ymin = min( min(deoxysignals(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(deoxysignals,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(deoxysignals(:)), max(max(boldinterps)) );
            ymin = min( min(deoxysignals(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, deoxysignals);
    ymax = max(deoxysignals(:));
    ymin = min(deoxysignals(:));
end
ylim([ymin,ymax]);
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(deoxysignals)
        deoxysignals{idx} = deoxysignals{idx}(TEidx, alphaidx);
    end
end
