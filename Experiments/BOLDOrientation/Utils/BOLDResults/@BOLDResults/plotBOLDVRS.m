function [ varargout ] = plotBOLDVRS( R, varargin )
%PLOT
...
[echotimes, alphas,boldsignalsVRS] = getboldplotargs(R,opts);
for idx = 1:numel(boldsignalsVRS)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,boldsignalsVRS{idx},opts);
end
...
function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,boldsignalsVRS,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(boldsignalsVRS(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
boldsignalsVRS = opts.scalefactor * boldsignalsVRS;
...
hold on
hx = plot(gca, echotimes, boldsignalsVRS, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',boldsignalsVRS.');
            ...
            ymax = max( max(boldsignalsVRS(:)), max(boldinterps(:)) );
            ymin = min( min(boldsignalsVRS(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(boldsignalsVRS,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(boldsignalsVRS(:)), max(max(boldinterps)) );
            ymin = min( min(boldsignalsVRS(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, boldsignalsVRS);
    ymax = max(boldsignalsVRS(:));
    ymin = min(boldsignalsVRS(:));
end
ylim([ymin,ymax]);
...
function [echotimes, alphas,boldsignalsVRS] = getboldplotargs(R,opts)
...
[echotimes, alphas, boldsignals, boldsignalsIntra, boldsignalsExtra, boldsignalsVRS] = getBOLDSignals(R,boldargs{:});
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(boldsignalsVRS)
        boldsignalsVRS{idx} = boldsignalsVRS{idx}(TEidx, alphaidx);
    end
end
