function [ varargout ] = plotDeoxyVRS( R, varargin )
%PLOT
...
for idx = 1:numel(deoxysignalsVRS)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,deoxysignalsVRS{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, deoxysignalsVRS);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,deoxysignalsVRS,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(deoxysignalsVRS(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
deoxysignalsVRS = opts.scalefactor * deoxysignalsVRS;
...
hold on
hx = plot(gca, echotimes, deoxysignalsVRS, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',deoxysignalsVRS.');
            ...
            ymax = max( max(deoxysignalsVRS(:)), max(boldinterps(:)) );
            ymin = min( min(deoxysignalsVRS(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(deoxysignalsVRS,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(deoxysignalsVRS(:)), max(max(boldinterps)) );
            ymin = min( min(deoxysignalsVRS(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, deoxysignalsVRS);
    ymax = max(deoxysignalsVRS(:));
    ymin = min(deoxysignalsVRS(:));
end
ylim([ymin,ymax]);
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(deoxysignalsVRS)
        deoxysignalsVRS{idx} = deoxysignalsVRS{idx}(TEidx, alphaidx);
    end
end
