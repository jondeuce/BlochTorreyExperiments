function [ varargout ] = plotBOLDExtra( R, varargin )
%PLOT
...
[echotimes, alphas, boldsignals,boldsignalsIntra,boldsignalsExtra] = getboldplotargs(R,opts);
for idx = 1:numel(boldsignalsExtra)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,boldsignalsExtra{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, boldsignalsExtra);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,boldsignalsExtra,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(boldsignalsExtra(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
boldsignalsExtra = opts.scalefactor * boldsignalsExtra;
...
hold on
hx = plot(gca, echotimes, boldsignalsExtra, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',boldsignalsExtra.');
            ...
            ymax = max( max(boldsignalsExtra(:)), max(boldinterps(:)) );
            ymin = min( min(boldsignalsExtra(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(boldsignalsExtra,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(boldsignalsExtra(:)), max(max(boldinterps)) );
            ymin = min( min(boldsignalsExtra(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, boldsignalsExtra);
    ymax = max(boldsignalsExtra(:));
    ymin = min(boldsignalsExtra(:));
end
ylim([ymin,ymax]);
...
function [echotimes, alphas, boldsignals,boldsignalsIntra,boldsignalsExtra] = getboldplotargs(R,opts)
...
[echotimes, alphas, boldsignals, boldsignalsIntra,boldsignalsExtra] = getBOLDSignals(R,boldargs{:});
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(boldsignalsExtra)
        boldsignalsExtra{idx} = boldsignalsExtra{idx}(TEidx, alphaidx);
    end
end
