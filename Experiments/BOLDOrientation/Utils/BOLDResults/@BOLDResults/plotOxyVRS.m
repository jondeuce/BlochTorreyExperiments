function [ varargout ] = plotOxyVRS( R, varargin )
%PLOT
...
for idx = 1:numel(oxysignalsVRS)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,oxysignalsVRS{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, oxysignalsVRS);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,oxysignalsVRS,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(oxysignalsVRS(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
oxysignalsVRS = opts.scalefactor * oxysignalsVRS;
...
hold on
hx = plot(gca, echotimes, oxysignalsVRS, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',oxysignalsVRS.');
            ...
            ymax = max( max(oxysignalsVRS(:)), max(boldinterps(:)) );
            ymin = min( min(oxysignalsVRS(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(oxysignalsVRS,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(oxysignalsVRS(:)), max(max(boldinterps)) );
            ymin = min( min(oxysignalsVRS(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, oxysignalsVRS);
    ymax = max(oxysignalsVRS(:));
    ymin = min(oxysignalsVRS(:));
end
ylim([ymin,ymax]);
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(oxysignalsVRS)
        oxysignalsVRS{idx} = oxysignalsVRS{idx}(TEidx, alphaidx);
    end
end
