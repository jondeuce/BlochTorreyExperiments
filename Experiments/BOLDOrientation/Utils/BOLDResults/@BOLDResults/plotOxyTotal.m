function [ varargout ] = plotOxyTotal( R, varargin )
%PLOT
...
for idx = 1:numel(oxysignals)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,oxysignals{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, oxysignals);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,oxysignals,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(oxysignals(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
oxysignals = opts.scalefactor * oxysignals;
...
hold on
hx = plot(gca, echotimes, oxysignals, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',oxysignals.');
            ...
            ymax = max( max(oxysignals(:)), max(boldinterps(:)) );
            ymin = min( min(oxysignals(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(oxysignals,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(oxysignals(:)), max(max(boldinterps)) );
            ymin = min( min(oxysignals(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, oxysignals);
    ymax = max(oxysignals(:));
    ymin = min(oxysignals(:));
end
ylim([ymin,ymax]);
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(oxysignals)
        oxysignals{idx} = oxysignals{idx}(TEidx, alphaidx);
    end
end
