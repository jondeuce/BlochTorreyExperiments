function [ varargout ] = plotOxyExtra( R, varargin )
%PLOT
...
for idx = 1:numel(oxysignalsExtra)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,oxysignalsExtra{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, oxysignalsExtra);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,oxysignalsExtra,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(oxysignalsExtra(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
oxysignalsExtra = opts.scalefactor * oxysignalsExtra;
...
hold on
hx = plot(gca, echotimes, oxysignalsExtra, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',oxysignalsExtra.');
            ...
            ymax = max( max(oxysignalsExtra(:)), max(boldinterps(:)) );
            ymin = min( min(oxysignalsExtra(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(oxysignalsExtra,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(oxysignalsExtra(:)), max(max(boldinterps)) );
            ymin = min( min(oxysignalsExtra(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, oxysignalsExtra);
    ymax = max(oxysignalsExtra(:));
    ymin = min(oxysignalsExtra(:));
end
ylim([ymin,ymax]);
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(oxysignalsExtra)
        oxysignalsExtra{idx} = oxysignalsExtra{idx}(TEidx, alphaidx);
    end
end
