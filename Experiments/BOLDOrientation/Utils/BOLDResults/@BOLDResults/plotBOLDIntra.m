function [ varargout ] = plotBOLDIntra( R, varargin )
%PLOT
...
[echotimes, alphas, boldsignals,boldsignalsIntra] = getboldplotargs(R,opts);
for idx = 1:numel(boldsignalsIntra)
    [~,~,fig(idx)] = plotBOLDSignal(R,echotimes,alphas,boldsignalsIntra{idx},opts);
end
...
switch nargout
    case 1
        varargout{1} = fig;
    case 3
        [varargout{1:3}] = deal(echotimes, alphas, boldsignalsIntra);
end

end

function [hbold,leg,fig] = plotBOLDSignal(R,echotimes,alphas,boldsignalsIntra,opts)

switch upper(opts.scalefactor)
    case 'NORMALIZE'
        opts.scalefactor = 1/max(boldsignalsIntra(:));
    case 'RELATIVE'
        opts.scalefactor = 1/prod(R.MetaData.SimSettings.VoxelSize);
end
boldsignalsIntra = opts.scalefactor * boldsignalsIntra;
...
hold on
hx = plot(gca, echotimes, boldsignalsIntra, 'x', 'markersize', 8);

if opts.interp
    switch upper(opts.interptype)
        case 'SPLINE'
            coeffs = spline(echotimes.',boldsignalsIntra.');
            ...
            ymax = max( max(boldsignalsIntra(:)), max(boldinterps(:)) );
            ymin = min( min(boldsignalsIntra(:)), min(boldinterps(:)) );
        case 'CHEBFUN'
            boldinterps = chebfun(boldsignalsIntra,[echotimes(1),echotimes(end)],'equi');
            figure(fig), hbold = plot(boldinterps);
            ymax = max( max(boldsignalsIntra(:)), max(max(boldinterps)) );
            ymin = min( min(boldsignalsIntra(:)), min(min(boldinterps)) );
    end
else
    figure(fig), hbold = plot(echotimes, boldsignalsIntra);
    ymax = max(boldsignalsIntra(:));
    ymin = min(boldsignalsIntra(:));
end
ylim([ymin,ymax]);
...
function [echotimes, alphas, boldsignals,boldsignalsIntra] = getboldplotargs(R,opts)
...
[echotimes, alphas, boldsignals, boldsignalsIntra] = getBOLDSignals(R,boldargs{:});
...
if any(~isinf([opts.minTE,opts.maxTE,opts.minAlpha,opts.maxAlpha]))
    ...
    for idx = 1:numel(boldsignalsIntra)
        boldsignalsIntra{idx} = boldsignalsIntra{idx}(TEidx, alphaidx);
    end
end
