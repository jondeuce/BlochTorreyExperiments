...
function runblocktorreygrid(;alpha = 0.5, PLOT = false)
    T = Float64
    V = Vec{2,T}
...
using StatsPlots
# plotly(size=(800,800), legend=nothing, labels=nothing) # html-based plotting in browser (nice, interactive, but slow)
# pyplot(size=(800,800), legend=nothing, labels=nothing) # interacting plotting gui (nice, but a bit slow)
gr(size=(800,800), legend=nothing, labels=nothing) # non-interactive plotting gui (fast, not interactive)

fd, fsubs, fh, h0, bbox, pfix = runblocktorreygrid(alpha = 0.1);
p, t = kmg2d(fd, fsubs, fh, h0/2.0, bbox, 1, 0, pfix;
    QMIN = 0.3,
    MAXITERS = 5_000,
    MPITERS = 500,
    FIXEDSUBSITERS = 250,
