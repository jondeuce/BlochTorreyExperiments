...
function runblocktorreygrid(;alpha = 0.5, PLOT = false)
    ...
    function dgrid(x::Vec{2,T}) where {T}
        ...
        @inbounds for i in eachindex(outercircles)
            douters = dunion(douters, dcircle(x, outercircles[i]))
            dinners = dunion(dinners, dcircle(x, innercircles[i]))
        end
    ...
    fsubs = [x->dintersect(drectangle0(x,rect), dcircle(x,c)) for c in Iterators.flatten(zip(outercircles, innercircles))]
