function [lambda, V, A] = laplacianSparseOp(varargin)

% LAPLACIANSPARSEOP   Sparse Negative Laplacian in 1D, 2D, or 3D
%
...
if nargout > 2 %also calulate the matrix if specified in the output
    ...
    if BB(1) == 2
        D1x(1,1) = 1;
    elseif BB(1) == 3
        D1x(1,end) = D1x(1,end)-1;
        D1x(end,1) = D1x(end,1)-1;
    end
    if BB(1+dim) == 2
        D1x(end,end) = 1;
    end
    ...
    if dim >= 2 && BB(2) == 2
        D1y(1,1) = 1;
    elseif dim >= 2 && BB(2) == 3
        D1y(1,end) = D1y(1,end)-1;
        D1y(end,1) = D1y(end,1)-1;
    end
    if dim >= 2 && BB(2+dim) == 2
        D1y(end,end) = 1;
    end
    ...
    if dim >= 3 && BB(3) == 2
        D1z(1,1) = 1;
    elseif dim >= 3 && BB(3) == 3
        D1z(1,end) = D1z(1,end)-1;
        D1z(end,1) = D1z(end,1)-1;
    end
    if dim >= 3 && BB(3+dim) == 2
        D1z(end,end) = 1;
    end
