function [f,s,m,mv,mvd,unA] = parallel_expmv_diffuse3D(t,A,b,M,prec,shiftmat,bal,full_term,prnt,force_estm,tol,h,D,f,muA,normA)
%PARALLEL_EXPMV_DIFFUSE3D Matrix exponential times vector or matrix.
...
A	=   @(u) fmg_diffuse(u,h,D,f); % A*x
cf	=   conj(f);
Ap	=   @(u) fmg_diffuse(u,h,D,cf); % A'*x
n   =	length(b);
...
if nargin < 4 || isempty(M)
   tt = 1;
   [M,mvd,alpha,unA] = select_taylor_degree_diffuse3D(A,[],[],prec,false,false,force_estm,Ap,n,normA,t,size(f,4));
   mv = mvd;
else
   tt = t; mv = 0; mvd = 0;
end
