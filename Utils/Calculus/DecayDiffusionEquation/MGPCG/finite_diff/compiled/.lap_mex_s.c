...
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	#pragma omp parallel for private(l, jx, jy, jz) collapse(3) schedule(dynamic, 32768)
    for(k = 0; k < N; k += nxny) {
        for(j = 0; j < nxny; j += nx) {
            for(i = 0; i < nx; ++i) {
                l  = k + j + i;
                ...
                if(Mask[l]) {
                    ...
                    dx[l] = (*func_pt[jz+jy+jx])(x, l, nx, nxny);
                }
            }
        }
    }
    
    if(fabsf(h - 1.0f) > 0.0001f) {
...
{
    return 2.0f*x[i] - 5.0f*x[i+1] + 4.0f*x[i+2] - x[i+3];
}

float
ofo(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return 2.0f*x[i] - 5.0f*x[i+nx] + 4.0f*x[i+2*nx] - x[i+3*nx];
}

float
oof(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return 2.0f*x[i] - 5.0f*x[i+nxny] + 4.0f*x[i+2*nxny] - x[i+3*nxny];
}
...
float
faa(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return x[i] - 2.0f*x[i+1] + x[i+2];
}

float
afa(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return x[i] - 2.0f*x[i+nx] + x[i+2*nx];
}

float
aaf(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return x[i] - 2.0f*x[i+nxny] + x[i+2*nxny];
}

float
baa(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return x[i] - 2.0f*x[i-1] + x[i-2];
}

float
aba(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return x[i] - 2.0f*x[i-nx] + x[i-2*nx];
}

float
aab(const float *x, uint32_t i, uint32_t nx, uint32_t nxny)
{
    return x[i] - 2.0f*x[i-nxny] + x[i-2*nxny];
}
