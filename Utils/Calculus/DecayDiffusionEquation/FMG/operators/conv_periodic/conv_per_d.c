...
void conv_per_dim2(double *x, double *g, const mwSize *xSize, const mwSize ndims, const mwSize *gSize, double *y );
void conv_per_dim3(double *x, double *g, const mwSize *xSize, const mwSize ndims, const mwSize *gSize, double *y );
...
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if(dim == 1) {
        ...
        conv_per_dim2(x,g,xSize,ndims,gSize,y);
    } else if(dim == 3) {
        conv_per_dim3(x,g,xSize,ndims,gSize,y);
    }
    ...
    for(k = 0; k < nxnynz; k += nxny) {
        for(j = 0; j < nxny; j += nx) {
            ...
            for(i = 0; i < len2; ++i, ++l, ++l2) {
                y[l] = x[l] * g[len2];
                for(m = 1; m <= i; ++m) {
                    y[l] += x[l+m] * g[len2+m] + x[l-m] * g[len2-m];
                }
                for(m = i+1; m <= len2; ++m) {
                    y[l] += x[l+m] * g[len2+m] + x[l2-m] * g[len2-m];
                }
            }
            ...
            for(i = NX; i > NX-len2; --i, --l, --l2) {
                ...
                for(m = NX-i+1; m <= len2; ++m) {
                    y[l] += x[l2+m] * g[len2+m] + x[l-m] * g[len2-m];
                }
            }
        ...
        for(i = 0; i < nx; ++i) {
            ...
            for(j = 0, j2 = 0; j < nxlen2; j += nx, ++j2, l += nx, l2 += nx) {
                ...
                for(m = 1, m2 = nx; m <= j2; ++m, m2 += nx) {
                    y[l] += x[l+m2] * g[len2+m] + x[l-m2] * g[len2-m];
                }
                for(m = j2+1; m <= len2; ++m, m2 += nx) {
                    y[l] += x[l+m2] * g[len2+m] + x[l2-m2] * g[len2-m];
                }
            }
            ...
            for(j = NY, j2 = 0; j > NY-nxlen2; j -= nx, ++j2, l -= nx, l2 -= nx) {
                ...
                for(m = j2+1; m <= len2; ++m, m2 += nx) {
                    y[l] += x[l2+m2] * g[len2+m] + x[l-m2] * g[len2-m];
                }
            }
