...
% J. Doucette	University of British Columbia, Vancouver Campus
% 2016 May 10
% 
% Modified to not require the need for 'bsx_times.m'; changes indicated
% by the comment '%%CHANGED - jd' next to the commented out code
...
disp ' TONY''S T.     GENOP  BSXFUN   BSXFUN2   BSXFUN3' %%CHANGED - jd
f9  = @() resh4D_clone(a, b);    % SX with Tony's trick
f10 = @() resh4D_genop(a, b);    % SX+TIMES with GENOP  (BSXFUN replacement)
% f11 = @() resh4D_bsxtimes(a, b); % SX+TIMES with BSXTIMES (BSXFUN replacement) %%CHANGED - jd
...
t = [t, timeit(f9),timeit(f10),timeit(f12),timeit(f13),timeit(f14)]; %%CHANGED - jd
disp (t(lenT+1:end))
...
% c = c0 - f11(); maxerror(11) = max(c(:)); %%CHANGED - jd
