...
@benchmark T2Dist.Rewrite.relaxmat_action!($(work.M), $num_states, $E2, $E1)

workf32, E2f32, E1f32 = map(x->Float32.(x), work), Float32(E2), Float32(E1);
@benchmark begin
    T2Dist.Rewrite.relaxmat_action!($(workf32.M), $num_states, $E2f32, $E1f32)
end
...
@benchmark begin
    T2Dist.Rewrite.flipmat_action!($(workf32.M), $num_states, $T2matf32);
end
...
@btime T2Dist.Rewrite.flipmat_action!($(work.M), $num_states, $T2mat);
