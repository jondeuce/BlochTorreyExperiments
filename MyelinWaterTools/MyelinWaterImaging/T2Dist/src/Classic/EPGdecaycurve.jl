...
"""
function EPGdecaycurve(ETL,flip_angle,TE,T2,T1,refcon)
    # Initialize magnetization phase state vector (MPSV) and set all
    # magnetization in the F1 state.
    M = complex(zeros(3*ETL));
    M[1] = exp(-(TE/2)/T2)*sin(deg2rad(flip_angle)/2);
    ...
    # Compute flip matrix
    T_1, T_p = flipmat(deg2rad(flip_angle), ETL, refcon);
    
    # Apply first refocusing pulse and get first echo amplitude
    M[1:3] = T_1*M[1:3];
    echo_amp[1] = abs(M[2])*exp(-(TE/2)/T2);
    ...
    for x = 2:ETL
        ...
        echo_amp[x] = abs(M[2])*exp(-(TE/2)/T2);
...
Compute the flip matrix as given by Hennig (1988), but corrected by Jones (1997)
"""
function flipmat(alpha,num_pulses,refcon)
    # Create element matrices
    alpha2 = alpha*refcon/180;
    T_1 = [   cos(alpha/2)^2   sin(alpha/2)^2 -im*sin(alpha);
              sin(alpha/2)^2   cos(alpha/2)^2  im*sin(alpha);
           -0.5im*sin(alpha) 0.5im*sin(alpha)     cos(alpha)]
    T_2 = [   cos(alpha2/2)^2   sin(alpha2/2)^2 -im*sin(alpha2);
              sin(alpha2/2)^2   cos(alpha2/2)^2  im*sin(alpha2);
           -0.5im*sin(alpha2) 0.5im*sin(alpha2)     cos(alpha2)];
    ...
    # end
    x = kron(ones(3*num_pulses), [1,2,3]) + kron(0:3:3*num_pulses-1, ones(9));
    y = kron(1:3*num_pulses, ones(3));
    v = kron(ones(num_pulses), T_2[:]);

    T_p = sparse(x,y,v,3*num_pulses,3*num_pulses);

    return @ntuple(T_1, T_p)
end
...
Hennig (1988)
"""
function relaxmat(num_states,te,t2,t1)

    #T_r = zeros(3*num_states,3*num_states);

    E2 = exp(-te/t2);
    E1 = exp(-te/t1);
    x = zeros(3*num_states-1);
    y = zeros(3*num_states-1);
    v = zeros(3*num_states-1);
    ...
    v[2:num_states] = E2*ones(num_states-1);
    # F(n) --> F(n+1)
    #for x = 1:num_states-1
    #    T_r(3*x+1,3*x-2) = exp(-te/t2);
    #end
    ...
    v[num_states+1:2*num_states-1] = E2*ones(num_states-1);
    # Z(n) --> Z(n)
    #for x = 1:num_states
    ...
    v[2*num_states:3*num_states-1] = E1*ones(num_states);
