"""
X, mu, Chi2Factor = LSQNONNEG_LCURVE(C, d)

Returns the regularized NNLS solution, X, by minimizing (C*X-d).^2 + mu*(H*X).^2.
H is the identity matrix and mu is chosen by the L-curve theory using the
Generalized Cross-Validation method. Chi2Factor is the ratio of the final chi^2
to the minimum chi^2 (mu = 0).
...
function lsqnonneg_lcurve!(work, C::AbstractMatrix{T}, d::AbstractVector{T}) where {T}
    ...
    mu = @timeit_debug TIMER "L-curve Optimization" begin
        opt_result = Optim.optimize(μ -> lcurve_GCV(μ, C, d, work), T(0), T(0.1), Optim.Brent(); rel_tol = T(1e-3))
