...
@inline_cufunc neglogL_rician(x, μx0, σx) = -_rician_logpdf_cuda(x, μx0, σx) # adjoint of `_rician_logpdf_cuda` is defined elsewhere
...
NegLogLKumaraswamy(x, α, β) = sum(_cap.(neglogL_kumaraswamy.(x, α, β))) / size(α,2) # Negative log-likelihood for Kumaraswamy (Note: sum over dims=1, mean over dims=2)
...
function _crossentropy_gradcheck_test()
    for T in [Float32, Float64]
        ...
        @assert gradcheck((x′, α, β) -> neglogL_kumaraswamy(Flux.σ(x′), α, β), randn(T), randn(T), randn(T); extrapolate = false, verbose = true)
