...
const settings = load_settings(joinpath(@__DIR__, "..", "settings", "hybrid_settings.toml"))
...
MMDloss(X,Y) = size(Y,2) * mmd_flux(models["logsigma"], corrected_signal_instance(X), Y) # m*MMD^2 on genatr data (`corrected_signal_instance` wraps the generator `models["G"]`)
...
function train_mmd_kernel!(
    ...
    function _sampleX̂Y()
        ...
        while true
            ...
            X̂ = corrected_signal_instance(X)
