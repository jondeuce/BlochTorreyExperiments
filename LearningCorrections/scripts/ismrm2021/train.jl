...
new_settings_template() = TOML.parse(
"""
[data]
    out    = "$(new_savepath())"
    ntrain = "auto" # 102_400
...
function make_signal_mask(Y; mincutoff::Int = 0)
    # Random mask such that only first `rand(mincutoff:size(Y,1))` signals are kept
...
function apply_signal_mask(Y; mincutoff::Int = 0)
    ...
    return Y .* make_signal_mask(Y; mincutoff)
end
...
function CVAElosses(Ymeta::AbstractMetaDataSignal; marginalize_Z)
    ...
    mask    = make_signal_mask(X̂; mincutoff)
    X̂masked = mask .* X̂
    ...
    ℓ = if λ_vae_reg == 0
        ...
        Ymasked = mask .* apply_pad(derived["cvae"], signal(Ymeta))
        zr_X̂    = sample_mv_normal(mv_normal_parameters(derived["cvae"], X̂masked))
        zr_Y    = sample_mv_normal(mv_normal_parameters(derived["cvae"], Ymasked))
        X̂_dec   = mask .* models["vae_dec"](zr_X̂)
        Y_dec   = mask .* models["vae_dec"](zr_Y)
        VAEReg  = λ_vae_reg * (sum(abs, X̂masked .- X̂_dec) + sum(abs, Ymasked .- Y_dec)) / size(Ymasked, 2)
        (; KLDiv, ELBO, VAEReg)
    end
...
function compute_metrics(engine, batch; dataset)
    @timeit "compute metrics" begin #TODO CUDA.@sync
        ...
        let
            ...
            accum!(img_key, MMDLearning.fast_hist_1D(Flux.cpu(vec(Y_fit_state.X̂)), img.meta[:histograms][dataset][0].edges[1]))
...
function makeplots(;showplot = false)
    try
        Dict{Symbol, Any}(
            :ricemodel    => plot_rician_model(logger, cb_state, phys; showplot, bandwidths = (filter(((k,v),) -> startswith(k, "logsigma"), collect(models)) |> logσs -> isempty(logσs) ? nothing : (x->Flux.cpu(permutedims(x[2]))).(logσs))),
            :signals      => plot_rician_signals(logger, cb_state, phys; showplot),
            :signalmodels => plot_rician_model_fits(logger, cb_state, phys; showplot),
            :infer        => plot_rician_inference(logger, cb_state, phys; showplot),
            :ganloss      => plot_gan_loss(logger, cb_state, phys; showplot, lrdroprate = settings["opt"]["lrrate"], lrdrop = settings["opt"]["lrdrop"]),
            :vallosses    => plot_all_logger_losses(logger, cb_state, phys; showplot, dataset = :val),
            :trainlosses  => plot_all_logger_losses(logger, cb_state, phys; showplot, dataset = :train),
