...
while G.iBVF + G.aBVF < G.Targets.BVF * ( 1.0 + BVF_RelErr )
    ...
    [ G.VasculatureMap, G.Idx{end+1} ]   =   getCylinderMask( G.GridSize, G.SubVoxSize, G.VoxelCenter, G.VoxelSize, ...
        G.P(:,end), G.Vz(:,end), G.R(end), G.Vx(:,end), G.Vy(:,end), isUnit, isCentered, prec, G.VasculatureMap );
...
% function [ G ] = ImproveMinorBVF_legacy( G )
% 
% % Ensure cylinder direction vectors are unit, and set parameters
% G           =   NormalizeCylinderVecs(G);
% isUnit      =   true;
...
% mx              =   cell(1,G.N);
% for ii = 1:G.Nmajor
%     [ VasculatureMap, mx{ii} ]    =    getCylinderMask( G.GridSize, G.SubVoxSize, G.VoxelCenter, G.VoxelSize, ...
%         G.p0(:,ii), G.vz0(:,ii), G.r0(ii), G.vx0(:,ii), G.vy0(:,ii), isUnit, isCentered, prec, VasculatureMap );
% end
% 
% % Add minor vasculature to Map
% for ii = G.Nmajor + (1:G.Nminor)
%     [ VasculatureMap, mx{ii} ]  =   getCylinderMask( G.GridSize, G.SubVoxSize, G.VoxelCenter, G.VoxelSize, ...
%         G.P(:,ii), G.Vz(:,ii), G.R(ii), G.Vx(:,ii), G.Vy(:,ii), isUnit, isCentered, prec, VasculatureMap );
% end
% 
% G.iBVF    =   BVF(VasculatureMap) - G.aBVF;
% 
% % We now have an initial guess for the G.iBVF; next we create more
% % cylinders until there are at least as many as are required
% BVF_RelErr  =   0.05 * (1.0 - G.Targets.BVF); % 5 percent relative to gap between G.Targets.BVF and 1.0
...
%     
%     if isInt
%         [p1,vz1,r1] = addIntersectingCylinders( G.VoxelSize, G.VoxelCenter, ...
%             R_Minor, [], isAA, false, G.p0, G.vz0, G.r0 );
%         [G.P,G.Vz,G.R] = deal([G.P,p1(:,end)],[G.Vz,vz1(:,end)],[G.R,r1(end)]);
%     else
%         [G.P,G.Vz,G.R]    =   nonIntersectingCylinders( G.VoxelSize, G.VoxelCenter, ...
%             R_Minor, [], isAA, false, G.P, G.Vz, G.R );
%     end
%     [G.Vx(:,end+1),G.Vy(:,end+1),G.Vz(:,end)] =   nullVectors3D( G.Vz(:,end) );
%     
%     [ VasculatureMap, mx{end+1} ]   =   getCylinderMask( G.GridSize, G.SubVoxSize, G.VoxelCenter, G.VoxelSize, ...
%         G.P(:,end), G.Vz(:,end), G.R(end), G.Vx(:,end), G.Vy(:,end), isUnit, isCentered, prec, VasculatureMap );
%     
...
% ind_List_CurrentBest    =   [];
% 
% Mean_Minor_BVF =   mean(cellfun(@numel,mx(G.Nmajor+1:end))) / prod(G.GridSize);
% Mean_Nminor    =   floor(Minor_BVF_Best/Mean_Minor_BVF);
% 
% loopiters = 0;
% loopiters_MAX = 10;
% while (loopiters < loopiters_MAX) && (abs(Minor_BVF_CurrentBest - Minor_BVF_Best) > 0.25 * (1/max(G.GridSize)) * Minor_BVF_Best)
%     loopiters = loopiters + 1;
%     MinorPerm = G.Nmajor + randperm(G.Nminor);
%     
...
%     iters = 0;
%     MinorIndsLeft = sort(MinorPerm(Mean_Nminor+1:end));
%     while (iters < 10) && (abs(Minor_BVF_CurrentBest - Minor_BVF_Best) > 0.25 * (1/max(G.GridSize)) * Minor_BVF_Best)
%         iters = iters + 1;
%         
%         count  = 0;
%         IndsLeft = MinorIndsLeft;
