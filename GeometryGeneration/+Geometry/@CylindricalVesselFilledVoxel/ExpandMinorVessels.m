function [ G_out, SpaceFactor ] = ExpandMinorVessels( G, VolumeFactor, TOLX, TOLF )
%ExpandMINORVESSELS This function expands the minor vessels away from (or
%towards) eachother in an isotropic way to achive an isotropic BVF which
%is `VolumeFactor` times the current target BVF. The process is
...
VFact = 0.5 + 0.5 * rand();
G = ExpandMinorVessels( Geom, VFact );
ShowBVFResults(G);
